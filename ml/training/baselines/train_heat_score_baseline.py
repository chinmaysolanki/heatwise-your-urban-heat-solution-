#!/usr/bin/env python3
"""
v1 baseline: regress ``heat_mitigation_score`` (same feature block as feasibility).

Swap target only; keep preprocessing contract identical for shared serving bundles later.
"""
...
from training.feature_registry import HEAT_MITIGATION_V1_FEATURES, LABEL_LEAKAGE_COLUMNS
from training.preprocess import add_species_primary_features, infer_schema
from training.target_registry import HEAT_MITIGATION_TARGET
...
def _build_xy(df: pd.DataFrame, species_csv: Path | None):
    ...
    cols = [c for c in HEAT_MITIGATION_V1_FEATURES if c in df.columns]
    leak = [c for c in cols if c in LABEL_LEAKAGE_COLUMNS]
    if leak:
        raise ValueError(f"label leakage into X: {leak}")
    X = df[cols].copy()
    y = df[HEAT_MITIGATION_TARGET.column].astype(np.float32).values
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Heat mitigation score baseline (HGBR).")
    ...
    ap.add_argument("--out-dir", type=Path, default=Path("runs/heat_mitigation_baseline"))
    args = ap.parse_args()
    ...
    report = {
        ...
        "target": HEAT_MITIGATION_TARGET.column,
        "train": regression_report(y_tr, pipe.predict(X_tr)),
        "val": regression_report(y_va, pipe.predict(X_va)),
        "test": regression_report(y_te, pipe.predict(X_te)),
    }
