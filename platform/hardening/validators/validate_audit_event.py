...
_SCHEMA = _ROOT / "schemas" / "audit_event_schema.json"
...
def validate_audit_event(payload: dict[str, Any]) -> ValidationResult:
    v = _validator()
    ...
    if pl is not None:
        if not isinstance(pl, dict):
            ...
            try:
                ...
                errs.append("payload is not JSON-serializable")

    ent_id = payload.get("entity_id")
    ent_type = payload.get("entity_type")
    if ent_id and not ent_type:
        errs.append("entity_type should be set when entity_id is present")
