...
import { computeCohortSummary } from "@/lib/services/cohortCommercialService";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  ...
  const cohortGranularity =
    req.query.cohortGranularity === "month" || req.query.cohortGranularity === "week"
      ? req.query.cohortGranularity
      : "week";

  const cohorts = await computeCohortSummary({
    windowStart: ws,
    windowEnd: we,
    region,
    projectType,
  ...
  return res.status(200).json({
    ...
    filters: { region, projectType, sourceChannel, cohortGranularity },
    cohorts,
  });
}
