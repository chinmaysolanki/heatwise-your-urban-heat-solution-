...
/**
 * Installer-oriented view: `installer` + `shared` sections plus optional InstallerExecutionSummary.
 */
export async function getInstallerSummaryPayload(
  dossierId: string,
): Promise<
  | { ok: true; report: ReportPayloadView; executionSummary: Record<string, unknown> | null }
...
> {
  ...
  const allowed = new Set(["installer", "shared"]);
  ...
  const exec = row.installerExecutionSummary;
  const executionSummary = exec
    ? {
        id: exec.id,
        execution: parseSectionPayload(exec.executionPayloadJson),
        readiness_checklist: exec.readinessChecklistJson ? JSON.parse(exec.readinessChecklistJson) : null,
        install_job_id: exec.installJobId,
      }
    : null;
